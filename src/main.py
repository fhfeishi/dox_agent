...
from .custom_tasks import CustomTaskStore, TaskConflict, TaskMissing
...
class TaskDraft(BaseModel):
    ...
    def valid_defaults(cls, value):
        reserved = {"task_id", "task_version", "corpus_id", "corpus_ids", "allowed_doc_ids",
                    "model", "resource_policy", "output_intent", "run_id"}
        if value is not None and (len(value) > 12 or any(
            not key.isidentifier() or len(key) > 40 or len(text) > 500 or key in reserved
...
def create_app(settings=None, knowledge=None, graph_factory=build_graph):
    ...
    async def chat(payload: ChatRequest, request: Request):
        ...
        if payload.task_id in builtin_ids:
            if payload.task_version is not None:
        ...
        task_defaults = task_definition.get("parameter_defaults", {}) if task_definition else {}
        ...
        if task_definition and any(
            not isinstance(client_params[key], str) or len(client_params[key]) > 500
            for key in task_defaults.keys() & client_params.keys()
        ):
            raise HTTPException(422, "自定义任务参数必须是 500 字以内的文本")
        effective_params = {**task_defaults, **client_params}
        effective_sources = {**{key: "task_default" for key in task_defaults}, **client_sources}
        for key in task_defaults.keys() & client_params.keys():
            effective_sources[key] = "user"
                        ...
                        async for event in graph.astream(
                            {
                                ...
                                "custom_task": ({**{key: task_definition[key] for key in ("background", "goal", "requirements")},
                                                 "parameters": {key: effective_params[key] for key in task_defaults}}
