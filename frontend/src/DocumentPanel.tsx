...
export function DocumentPanel({ open, onClose, header, meta, children }: {
  ...
  return <div className="fixed inset-0 z-40 flex justify-end lg:pointer-events-none">
    ...
    <div role="dialog" aria-modal="true" aria-label={"文档预览：" + header} className="relative flex h-full w-full max-w-3xl flex-col bg-white shadow-xl lg:pointer-events-auto lg:m-4 lg:h-[calc(100dvh-2rem)] lg:w-1/2 lg:min-w-[34rem] lg:max-w-[min(60rem,90vw)] lg:rounded-2xl lg:shadow-2xl">
      <div className="flex items-start justify-between gap-3 border-b border-stone-200 p-4 lg:px-6">
      ...
      <div className="min-h-0 flex-1 overflow-auto p-5 lg:p-6">{children}</div>
    </div>
  </div>;
}
