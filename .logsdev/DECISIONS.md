...
## 图文报告复用本地解析图，并与原文版本一起保存（2026-09-25，用户要求；规划待实施）

- 选择：不另开“对整份 PDF 抽取所有图片”的生成管线。优先用现有 `parsed/<相对源文件>/middle_json.json` 的页号、图块、图注、bbox 与真实 `image_path`，仅在本次报告实际入模的文档中挑选少量相关 chart/image；缓存缺失或多联图需要完整裁图时，才按原 PDF 页与块定位定点裁取。MinerU 官方输出契约明确 ZIP 图片和页/块索引随解析产物保存，且命名可能变化，故不得猜文件名：[输出契约](https://github.com/opendatalab/MinerU/blob/master/docs/en/reference/output_files.md)。
- 理由：2026-09-25 只读检查在 133 份解析结果中发现 7057 张 JPG，抽看包含研究曲线、封面标志和表格截图；“有图片”不能推出“适合报告”。同页多 chart 可共用尾部图注，必须先处理图组，避免错配。现有检索仅剥离图像字节，报告/成果/DOCX 不保存图片及其来源，故代码提交和解析缓存不能冒充图文报告已实现。
- 约束：保留既定 A 管线——Markdown 正文 + 内容寻址图片附件构成不可变成果版本，DOCX/预览/含图 Markdown ZIP 均由同一版本派生；图片绑定库、文档及版本、源 PDF 页、图块与图片哈希，旧成果附件随状态成组备份。自动选图仅在“图文”需求下启动，不要求用户逐张审批；未核对的图、标志/表单/表格截图和不完整多联图不自动插入，缺图降为有说明的文字报告。原有纯文本报告和 `.md` 导出兼容。样本门槛与验收唯一维护于 ITERATION §16.29。
