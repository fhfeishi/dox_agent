...
- 最近核对：2026-09-24（W3-B 已随 `0c57a0f` 提交；W4-A 问答/对比自定义任务主链已实施，检查与缺口见 ITERATION §16.19。W4-B 自定义报告模板与 W4-C 生命周期、W5–W7、真实模型/OCR 仍待验）。
...
W4-A **已实施首切片**：复制内置 task1/task2 为自定义问答/对比任务，编辑草稿、发布不可变版本、在会话中固定版本并于运行快照保留参数来源。类型化参数、完整四段编辑器和归档仍待做。**已采纳、待实施**的 W4-B 方向是复制四份只读内置报告模板为带版本的自定义 Markdown 章节模板，并接入报告型任务。W4 不引入新执行引擎、联网或 Skill 权限；对象与验收见 [`ITERATION.md`](ITERATION.md) §16.18–16.19。
