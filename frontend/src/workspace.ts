...
export type SessionData = { turns: Turn[]; options: Options; archived?: boolean; branches?: Branch[]; task_id?: string; source_session_id?: string; source_turn_index?: number };
...
export function useWorkspace(turns: Turn[], options: Options, setTurns: Dispatch<SetStateAction<Turn[]>>, setOptions: (options: Options) => void,
  ...
  async function saveNow(snapshot = turnsRef.current, effectiveOptions = optionsRef.current, branchSnapshot = branchesRef.current, taskSnapshot = taskIdRef.current) {
    ...
    await enqueue({ id: activeRef.current, revision: revisions.current[activeRef.current] ?? 0,
      ...
      data: { ...existing?.data, turns: snapshot, options: effectiveOptions, branches: branchSnapshot, task_id: taskSnapshot } });
  }
  useEffect(() => {
    ...
    pending.current = { id: active, revision: revisions.current[active] ?? 0,
      title: existing?.title ?? turns[0].question.slice(0, 100), data: { ...existing?.data, turns, options, branches, task_id: taskIdRef.current } };
  ...
  async function select(id?: string, task?: string) {
    await flush();
